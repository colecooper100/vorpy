...
cpu_rnt = biot_savart_nonuniform_trapezoidal_rule(STEPSIZE,
                                                    sfp,
                                                    svpp1,
                                                    svpp2,
                                                    crads[1],
...
function gpu_kernel(vec_rtn,  # return values
                        stepsize, fp, vpp1, vpp2, crad1, crad2, circ1, circ2)  # input values
    
    vec_rtn .= biot_savart_nonuniform_trapezoidal_rule(stepsize, fp, vpp1, vpp2, crad1, crad2, circ1, circ2)
...
gpu_vec_rtn = CuArray{Float32}(undef, 3)

# Run the kernel
# @device_code_warntype
# @device_code_llvm
@device_code_warntype @cuda gpu_kernel(gpu_vec_rtn,
                                        STEPSIZE, sfp, svpp1, svpp2, crads[1], crads[2], circs[1], circs[2])

println("GPU return: ", gpu_vec_rtn)