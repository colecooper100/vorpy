...
# statement here**
include("nonuniform_trapezoidal_rule_wbs.jl")
...
function ana_velocity_polygonal_model(fp::SVector{3, T},
                                                vpp1::SVector{3, T},
                                                vpp2::SVector{3, T},
                                                circ1::T,
...
function bimodal_integrator_polygonal_path(stepsize::T,
    ...
    if pathincutoff[1]
        ...
        # error).  
        velincutoff = nonuniform_trapezoidal_rule_wbs(stepsize, fp, pathincutoff[2][1], pathincutoff[2][2], crad1, crad2, circ1, circ2)
    end
    ...
    for elm in pathoutcutoff
        if elm[1]
            veloutcutoff = veloutcutoff .+ ana_velocity_polygonal_model(fp, elm[2][1], elm[2][2], circ1, circ2)
        end
    end
