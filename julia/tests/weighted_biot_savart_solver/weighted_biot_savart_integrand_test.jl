...
sum_wbsintegrand = zeros(3)
sum_volgradtensor = zeros(3, 3)
for ell in ENDPOINTS[1]:1:ENDPOINTS[2]
    # println("ell = ", ell)  # DEBUG
    rtn = weighted_biot_savart_integrand(Float32(ell),
                                                sfp,
                                                svpp1,
                                                svpp2,
                                                crads[1],
                                                ...
                                                circs[1],
                                                circs[2])
    sum_wbsintegrand .+= cpu_rnt[1]
    sum_volgradtensor .+= cpu_rnt[2]
end

println("sum_wbsintegrand = ", sum_wbsintegrand)  # DEBUG
println("sum_volgradtensor = ", sum_volgradtensor)  # DEBUG
println("Spin matrix = ", 0.5 * (sum_volgradtensor - transpose(sum_volgradtensor)))  # DEBUG



###### Run on the GPU ######
# Make a wrapper to run functions on the GPU
function gpu_kernel(integrand_vec_rtn, velocity_gradient_tensor_rtn, ell_rtn, endofseg,  # return values
    ...
    velocity_gradient_tensor_rtn .= rtn[2]
    ell_rtn .= rtn[3]
    endofseg .= rtn[4]
...
gpu_velocity_gradient_tensor_rtn = CuArray{Float32}(undef, 3, 3)
...
# @device_code_llvm
@device_code_warntype @cuda gpu_kernel(gpu_integrand_vec_rtn, gpu_velocity_gradient_tensor_rtn, gpu_ell_rtn, gpu_endofseg,
                                        ELL, sfp, svpp1, svpp2, crads[1], crads[2], circs[1], circs[2])

println("GPU return: ", gpu_integrand_vec_rtn, ", ", gpu_velocity_gradient_tensor_rtn, ", ", gpu_ell_rtn, ", ", gpu_endofseg)
